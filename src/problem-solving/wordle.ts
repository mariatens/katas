export function wordle(guessWord: string, solutionWord: string): string[] {
  ...
  solutionArray.forEach((letter, i) => {
    //for each letter of the solution
    if (guessWord[i] === letter) {
      //if the letter of the guessed word is the same as the one from solution
  ...
  for (let i = 0; i < guessWord.length; i++) {
    if (solutionArray.includes(guessWord[i])) {
      //if the solution includes the letter in that index
